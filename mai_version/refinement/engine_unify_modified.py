...
"""
# from problog.__future__ import print_function
from problog.errors import GroundingError
from problog.logic import is_variable, Term
...
def unify_call_return(result, call_args, context, var_translate, min_var, mask=None):
	...
	for r, c, m in zip(result, call_args, mask):
		if m:
			...
			unify_value_different_contexts(c, r, sv, tv)
	min_var = sv.min_var
...
def subsumes(generic: Term, specific: Term) -> bool:
	"""
	Check whether generic subsumes specific
	:param generic:
	:param specific:
	:return:
	"""

	source_values = _VarTranslateWrapper({}, 0)
	target_values = {}  # dict: internal mapping in caller

	# Unify both terms.
	unify_value_different_contexts(generic, specific, source_values, target_values)

	# tv contains all bindings to variables in specific (to constants, or to values within tv).
	# There should be none.
	return not target_values